...
import "../../CareneederComponent/CareneederSchedule/CareneederSchedule.css";
...
const AnimalcareneederSchedule: React.FC = () => {
  const [schedule, setSchedule] = useState<AnimalcareneedersSchedule>({
    id: 0, 
    scheduletype: "长期",
    totalhours: "", 
    frequency: "每天", 
  ...
  const animalcareneederformId = queryParams.get("animalcareneederformId");
  ...
  const handleSubmit = () => {
    ...
    const API_URL = `${BASE_URL}/api/animalcareneeder/animalcareneeder_schedule`; // Replace with your actual API endpoint

    const requestData = {
      animalcareneederform_id: animalcareneederformId,
    ...
    fetch(API_URL, {
      ...
      .then((data) => {
        ...
        const animalcareneederformId = data.animalcareneederform_id;
        // You can show a success message to the user or navigate to another page

        navigate(
          `/signup_animalcareneeder/details/schedules/ads?animalcareneederformId=${animalcareneederformId}&phone=${phoneNumber}`
        );
      })
      .catch((error) => {
...
export default AnimalcareneederSchedule;
