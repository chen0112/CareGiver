...
interface AnimalCaregiverFormProps {
  API_URL: string;
  API_URL_UPLOAD: string;
  updateCaregivers: (newCaregiver: AnimalCaregiverForm) => void;
  getCaregivers: () => void;
}
...
const AnimalCaregiverWebForm: React.FC<AnimalCaregiverFormProps> = ({
  API_URL,
  API_URL_UPLOAD,
  updateCaregivers,
  getCaregivers,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    fetch(API_URL, {
      ...
      .then((response) => response.json())
      .then((newCaregiver) => {
        console.log("New_Animal_Caregiver:------", newCaregiver);
        ...
        const animalcaregiverformId = newCaregiver.id;

        resetForm();
        setTimeout(() => {
          navigate(
            `/signup_animalcaregiver/details?animalcaregiverformId=${animalcaregiverformId}&phone=${formData.phone}`
          );
        }, 2000);
      })
...
export default AnimalCaregiverWebForm;
